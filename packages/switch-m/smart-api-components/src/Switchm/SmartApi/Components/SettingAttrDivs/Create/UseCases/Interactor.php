...
class Interactor implements InputBoundary
{
    ...
    {
        // 表示順情報取得
        $result = $this->attrDivDao->getDisplayOrder($inputData->division())['list'];

        // 既に10件以上登録されている場合はエラー
        if (10 <= count($result)) {
        ...
        // codeを生成（日時+ミリ秒+member_id）
        $arrTime = explode('.', microtime(true));
        $code = date('YmdHis', $arrTime[0]) . substr($arrTime[1], 0, 3) . $inputData->id();

        // 並び順
        $displayOrder = 0;

        if (0 < count($result)) {
            $displayOrder = $result[0]->display_order + 1;
        }
